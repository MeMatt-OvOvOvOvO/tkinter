...
def onclick():
    ...
    c = entries[2].get()
    l = tk.Label(window)
    l.grid(row=4, column=1, rowspan=2, columnspan=3)
    ...
    if a == '' or b == '' or c == '':
        ...
        if isinstance(qF.roots[0], str):
            ...
            l["text"] = "Podana funkcja ma 1 miejsce zerowe\n w " + f"{qF.roots[0]:g}"
        else:
            l["text"] = "Podana funkcja ma 2 miejsce zerowe\n w " + f"{qF.roots[0]:g}" + " i " + f"{qF.roots[1]:g}"
...
b = tk.Button(window, command=onclick, text="Click to calculate roots")
